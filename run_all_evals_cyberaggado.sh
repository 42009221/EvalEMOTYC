#!/usr/bin/env bash
# Run the 8 CyberAggAdo EMOTYC evaluation configurations.
# Dataset: CyberAggAdo using the global total gold file only.
...
CONFIG_NAMES=(
  ...
  "NoContextTemplateSansEspaceMode005"
  "NoContextTemplateAvecEspace"
  "NoContextTemplateAvecEspaceMode005"
  "ContextTemplateSansEspace"
  "ContextTemplateSansEspaceMode005"
  "ContextTemplateAvecEspace"
  "ContextTemplateAvecEspaceMode005"
)
...
printf '=%.0s' {1..80}; echo
printf 'EMOTYC CyberAggAdo batch evaluation\n'
printf '  python       : %s\n' "$PYTHON_BIN"
printf '  script       : %s\n' "$PREDICT_SCRIPT"
printf '  gold         : %s\n' "$GOLD_FILE"
...
echo
printf '=%.0s' {1..80}; echo
printf 'CyberAggAdo batch complete in %ss\n' "$elapsed"
printf 'Results base: %s\n' "$RESULTS_BASE"
printf '=%.0s' {1..80}; echo
